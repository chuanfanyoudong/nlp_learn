...
import numpy as np
from jiang_classfication.lstm.deep_data import load_data
...
def main(**kwargs):
    if not torch.cuda.is_available():
        torch.manual_seed(777)  # set random seed for cpu

    train_iter, val_iter, test_iter, vocab_size, vectors = load_data()
    ...
    best_model_path = os.path.join(args.save_dir, 'article{}_{}_{}.pth'.format(args.model, args.text_type, best_score))
    ...
    if not os.path.exists('result/'):
        os.mkdir('result/')
    result_path = 'result/' + 'article{}_{}_{}'.format(args.model, args.id, args.best_score)
    val_probs, val_pred = val_result(model, val_iter, args)
    val_pred[['id', 'class']].to_csv('{}.csv'.format("val" + result_path ), index=None)
...
def val_result(model, val_data, args):
    # 生成测试提交数据csv
    # 将模型设为验证模式
    model.eval()
...
def _test(model, test_data, args):
    # 生成测试提交数据csv
    # 将模型设为验证模式
    model.eval()
    ...
    test = pd.read_csv('/data/users/zkjiang/test/data_grand/data/test_set.csv')

...
MPDEL_PATH = ROOT + __config["traditional_ml"]["model_path"]
...
def fasttext_predict(sentence):
    ...
    if model == None:
        print("加载fasttext模型")
        model = load_model(MPDEL_PATH + "cooking.bin")
    start_time = time.time()
    result =  model.predict(sentence)[0][0][-1]
    end_time = time.time()
...
if __name__ == '__main__':
    sentencee = "今天篮球打的好开心"
    print(fasttext_predict(sentencee))
#
# if __name__ == "__main__":
#     ## 配置输入数据的路径

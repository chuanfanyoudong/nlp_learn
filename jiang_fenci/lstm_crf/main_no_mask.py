...
def main():
    ...
    for epoch in range(int(__config["segment"]["epoch"])):
        ...
        for sentence, tags in train_data:
            ...
            optimizer.step()
            total_loss += loss
        print("正在进行第{}次迭代, loss为{}".format(epoch, total_loss))
        total_loss = 0.0
        with torch.no_grad():
            inputs = prepare_sequence(train_data[0][0], word2id)
            tags_in = prepare_sequence(train_data[0][1], tag2id)
            ...
            print(np.argmax(tag_scores, axis=1))
        print("训练集上的结果：")
        val(lstm_split, train_data, word2id, tag2id)
        print("测试集上的结果：")
...
def val(model, val_data, word2id, tag2id):
    ...
    print(metrics.flat_f1_score(all_tags, all_pre_tags, average='weighted', labels=[0, 1, 2, 3]))
